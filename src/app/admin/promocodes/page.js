...
export default function PromocodesPage() {
  ...
  const handleEdit = (promocode) => {
    ...
    setFormData({
      ...
      cardIds: [], // TODO: загрузить cardIds при необходимости
              ...
              {promocodes.map((promo) => (
                  <tr key={promo.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 space-x-2">
                      <button onClick={() => handleEdit(promo)} className="text-blue-600 hover:text-blue-900">Редактировать</button>
                      <button onClick={() => handleDeletePromocode(promo.id)} className="text-red-600 hover:text-red-900" disabled={promo.current_uses > 0}>Удалить</button>
                    </td>
                  </tr>
              ))}
              </tbody>
                ...
                <form onSubmit={(e) => { e.preventDefault(); editingPromocode ? handleUpdatePromocode() : handleCreatePromocode(); }}>
                  <div className="mb-4">
                    ...
                    <div className="flex gap-2">
                      <input type="text" required value={formData.code} onChange={(e) => setFormData(prev => ({ ...prev, code: e.target.value.toUpperCase() }))} className="flex-1 px-3 py-2 border rounded-md"/>
                      <button type="button" onClick={handleGenerateCode} className="px-3 py-2 bg-gray-600 text-white rounded-md">🎲</button>
                    </div>
                  </div>
                  <div className="mb-4">
                    <label className="block text-sm font-medium mb-2">Описание</label>
                    <textarea value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} className="w-full px-3 py-2 border rounded-md" rows="3"/>
                  </div>
                  <div className="mb-4">
                  ...
                  <div className="flex gap-3">
                    <button type="submit" className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-md">{editingPromocode ? 'Обновить' : 'Создать'}</button>
                    <button type="button" onClick={() => { setShowCreateModal(false); setEditingPromocode(null); resetForm(); }} className="flex-1 bg-gray-600 text-white px-4 py-2 rounded-md">Отмена</button>
                  </div>
                </form>
              </div>
            </div>
